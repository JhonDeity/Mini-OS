...
void ignore_signal(int signo) {
    // Función que ignora SIGINT (Ctrl+C)
}
...
int main() {
    ...
    // El proceso padre (la shell) ignora Ctrl+C para no cerrarse accidentalmente.
    ...
    while (1) {
        // 1. Recolección de procesos en segundo plano (Manejo de SIGCHLD/Zombies)
        ...
        // Comando interno: salir
        if (strcmp(args[0], "salir") == 0) {
            printf("Saliendo ...\n");
            break;
        }

        // Comando interno: cd (cambiar de directorio)
        if (strcmp(args[0], "cd") == 0) {
            ...
            continue; //Salta el fork y vuelve al prompt
        }
        ...
        if (pid < 0) {
            ...
            // Revertir manejo de SIGINT: Los hijos no deben ignorar Ctrl+C
            signal(SIGINT, SIG_DFL);
            
            if (has_pipe) {
                // Lógica de PIPE (doble fork dentro del hijo)
                int fd[2];
                if (pipe(fd) == -1) { perror("pipe"); exit(1); }
                
                pid_t pid2 = fork();
                if (pid2 == 0) {
                    // Primer comando (cmd1) - Escritor
                    close(fd[0]); 
            ...
            // Aplicar Redirecciones (solo si NO hay pipe)
...
/**
 * Recolecta procesos hijo terminados (zombies) de forma no bloqueante.
 */
void wait_for_background_jobs() {
    pid_t pid;
    int status;
